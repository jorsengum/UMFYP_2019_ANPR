...
MAX_NUMBER_OF_MATCHING_CHARS = 7
...
###################################################################################################
def loadKNNDataAndTrainKNN(classificationTxt, flattenImages):
    allContoursWithData = []                # declare empty lists,
    validContoursWithData = []              # we will fill these shortly

    try:
        npaClassifications = np.loadtxt(classificationTxt, np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt(flattenImages, np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
    ...
    for possibleChar in listOfPossibleChars:                                                    # for each possible char in the one big list of chars
        ...
        if len(listOfMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS or len(listOfMatchingChars) > MAX_NUMBER_OF_MATCHING_CHARS:     # if current possible list of matching chars is not long enough to constitute a possible plate or too long
            continue                            
...
def findListOfMatchingChars(possibleChar, listOfChars):
    # Find all characters and match in the macthing list and return as a list
    listOfMatchingChars = []                
...
def angleBetweenChars(firstChar, secondChar):
    ...
    if fltAdj != 0.0:                                   # skip to prevent division by 0
...
###################################################################################################
# Character Recognition, The role of this character recognition is to provide a more accurate estimate of 
# how manay characters in a candidate plate region, the actual recognition will be performed using Tesseract instead after another round of preprocessing
# on the extracted plates
